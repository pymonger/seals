...
L.TileLayer = L.Class.extend({
	...
	_getTilePos: function (tilePoint) {
		...
		return tilePoint.multiplyBy(tileSize).subtract(origin);//.subtract({x:tileSize,y:tileSize});
	},
	...
	_getWrapTileNum: function (tilePoint) {
		// TODO refactor, limit is not valid for non-standard projections
		//real_row = tile_row + Pow(2, zoomLevel -2);
		//console.log('tilepoint is',tilePoint)
		/*console.log('this is',this);
		var kids = [];
		$.each(this['_container'].children,function(i,child){
			//console.log('kids',child)
			kids.push(child);
		})
		console.log('image src is ',kids)*/
		/*var tile = this._tiles[tilePoint.x + ':' + tilePoint.y];
		var foo = $($(tile).context).clone()
		var canReturn = false;
		var kv = {};
		setTimeout(function(){
			if(typeof tile != "undefined") console.log('mh tile is ',$(foo.context).attr('src'));
			if(typeof($(foo.context).attr('src')) != "undefined"){
				console.log('src is',$(foo.context).attr('src'))
				var src = $(foo.context).attr('src').split('?')[1].split('&');
		...
		/*},1)*/
//console.log('tiles',this._tiles)
		if(typeof this._alltileMatrix != "undefined" && typeof this._alltileMatrix[tilePoint.x + ':' + tilePoint.y] != "undefined"){
			var tile = this._alltileMatrix[tilePoint.x + ':' + tilePoint.y];
			//var f = $($(tile).context).clone();
			//console.log('f si',tile.meta)
			if(typeof tile.meta != "undefined")
				return tile.meta['TILEROW'] + Math.pow(2, this._getZoomForUrl()-2 );
			else 
	...
	_returnDelayedTiles: function(kv){
		/*kind of like an interval to see if our keys and vals 
		  of the tile image src are valid, as it takes a 0001ms delat to read in _getWrapTileNum */
		//if()
	},
	...
	_loadTile: function (tile, tilePoint) {
		...
		var kv = {};
		var src = tile.src.split('?')[1].split('&');
		...
		//console.log(kv);
		$(tile).attr('data-tilerow',kv['TILEROW']);
		$(tile).attr('data-tilecol',kv['TILECOL']);
		$(tile).attr('data-tilematrix',kv['TILEMATRIX'])
		//console.log('tp',tile)
	},
