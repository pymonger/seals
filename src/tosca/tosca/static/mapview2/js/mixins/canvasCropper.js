	...
	function(){
		function canvasCropper(){
			this.triggerHoverTip = function(meta,left,top){
				...
				block.append('<li>Name: <a class="select_dataset">'+name+'</a></li>');
				...
				this._map.on('drag',function(e){
					$('#imageTooltip').hide();
                                ...
                                $('a.select_dataset').on('click',function(e){
					$('.facetview_freetext').val('_id:"'+name+'"');
			...
			this.constructCroppedImage = function(img,hit,hitId,bounds){
				...
				myImage.crossOrigin = "anonymous";
                                ...
                                // add tile
                                var tiles = hit.fields.metadata.tiles;
                                var tile_layers = hit.fields.metadata.tile_layers;
                                var tilesTmpl = null;
                                if (tiles) {
                                        var prodBase = hit.fields.urls[0];
                                        $.ajax({
                                            //async: false,
                                            method: "GET",
                                            url: prodBase,
                                            ...
                                            success: function(data, sts, xhr) {
                                                //console.log("prodBase data: " + data);
                                                tileTmpl = data + '/tiles/{ds}/{z}/{x}/{y}.png';
                                                //for (var i=0; i < tile_layers.length; i++) {
                                                    that._tileLayers[img] = new L.tileLayer(tileTmpl, {
                                                                ds: tile_layers[tile_layers.length-1],
                                                                opacity: .8,
                                                                minZoom: 0,
                                                                maxZoom: 18,
                                                                maxNativeZoom: 8,
                                                    ...
                                                    that._tileLayers[img].addTo(that._prodLayer);
                                                //}
                                            },
				...
				myImage.onload = function(e){
					var sI = setInterval(function(){
						if(myImage.width > 0) {
							...
							//that._imageLayers[img] = new L.ImageOverlay(img,bounds, { opacity:0 });
							that._imageLayers[img] = new L.ImageOverlay(img,bounds, { opacity: 0 });
							...
							L.DomEvent.on(that._imageLayers[img]._image, 'click', function(e) {
								...
								console.log('canvasCropper meta1 is',meta);
							...
							L.DomEvent.on(that._imageLayers[img]._image, 'mouseover', function(e) {
								...
								$(that._imageLayers[img]._image).addClass('ishovered')
						            //$.each(that._tileLayers,function(i,x){
                                                            ...
                                                            //        console.log("mouseover imgid: ", _id);
                                                            //        x.setOpacity(0);
                                                            //        console.log('hiding tile',x);
                                                            //    }
                                                            //    that._tileLayers[_id].bringToFront();
                                                            //    that._tileLayers[_id].setOpacity(1);
                                                            //});
							...
							imag.on('click',function(e){
								...
								var meta = that._imageMeta[_id];
								var imgOverlay = that._imageLayers[img];
								console.log('canvasCropper meta2 is',meta);
								var coords = meta.fields.metadata.center.coordinates;
								var ll = new L.LatLng(coords[1],coords[0]);
								var tileObj = $($('#map_'+_id).parents('tr')[1]).clone();
								var offs = $(this).offset();
